 ...
 */
 ...
 void battery_timer_expiry(struct k_timer *timer_id)
 {
	 ...
	 if (batt_mV >= 0) { // Check for valid reading
 ...
 static void bmi270_handler_func(void *unused1, void *unused2, void *unused3)
 {
	 ...
	 while (1) {
		 ...
		 if (k_sem_take(&bmi270_isr_sem, K_FOREVER) == 0) {
			 ...
			 uint8_t acc_range = (state == STATE_HOME_ADVERTISING) ?
											 bmi270_acc_config_high.acc_range :
											 bmi270_acc_config_low.acc_range;
			 uint8_t gyr_range = (state == STATE_HOME_ADVERTISING) ?
											 bmi270_gyr_config_high.gyr_range :
											 bmi270_gyr_config_low.gyr_range;
